...
{
    public partial class frDieuChinhHoaDon : Form
    {
        CAPNUOC_TNCEntities db = new CAPNUOC_TNCEntities();
        int type = 0;
        public frDieuChinhHoaDon()
        {
            InitializeComponent();
                ...
                if (rs == DialogResult.OK)
                {
                    ...
                    var hoadon = db.HOADONs.Where(x => x.SO_HD == SOHD && x.trangthai_id == 1 && x.KY_HIEU_HD == cboKyHieu.Text).FirstOrDefault();
                        ...
                        if (hoadon != null && hoadon.ghichu != "Đã điều chỉnh thông tin")
                        {
                            ...
                            {
                                MessageBox.Show("Có lỗi trong quá trình xử lý! Mã lỗi: " + result);
        ...
        {
            cboDieuchinh.DropDownStyle = ComboBoxStyle.DropDownList;
