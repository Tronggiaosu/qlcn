...
{
    public partial class UcUserForm : View.Core.NovUserControl
    {
        private CAPNUOC_TNCEntities db = new CAPNUOC_TNCEntities();
        private decimal nguoidung_id = 0;
        private string manv = "";
        ...
        private void editButton_Click(object sender, EventArgs e)
        {
            try
            {
                if (manv == "")
                {
                    MessageBox.Show("Chưa chọn nhân viên để tạo tài khoản!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                if (!IsDataOK()) return;
                if (nguoidung_id == 0) //chưa có tài khoản
                {
                    ...
                    db.SaveChanges();
                    MessageBox.Show("Thêm mới thành công! Mật khẩu mặc định: 123456", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else //edit
                {
                    var nhanvien = db.NHANVIENs.Where(x => x.maNV == maNVtextBox.Text).FirstOrDefault();
        ...
        private void resetPassButton_Click(object sender, EventArgs e)
        {
            ...
            nd.pass = md5("123456");
            db.SaveChanges();
            MessageBox.Show("Reset mật khẩu thành công, mật khẩu mặc định: 123456", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
