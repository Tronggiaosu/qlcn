...
{
    public partial class UcQLHoaDonThayThe : View.Core.NovUserControl
    {
        private CAPNUOC_TNCEntities db = new CAPNUOC_TNCEntities();

        public UcQLHoaDonThayThe()
        {
            InitializeComponent();
            btnReplace.Click += btnAdjust_Click;
        ...
        private void txtTim_KeyDown(object sender, KeyEventArgs e)
        {
            if (txtTim.Text != "")
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    var hoadon = db.HOADONs.Where(x => x.trangthai_id == 9).OrderByDescending(x => x.ID_HD).ToList();
                    var hoadonDC = (from hd in db.HOADONs
                                    from dc in db.HOADONs
                                    where hd.ID_HD == dc.keys && hd.trangthai_id == 9
                                    select dc
        ...
        private void seachButton_Click(object sender, EventArgs e)
        {
            ...
            var dataSourceDGV1 = hoadon.Select(x => new { x.ID_HD, x.KHACHHANG.hoten_KH, x.DANHBO, x.MaLT, x.SO_HD, chitiet = "Chi tiết" });
            var dataSourceDGV2 = hoadonDC.Select(x => new { x.ID_HD, x.KHACHHANG.hoten_KH, x.DANHBO, x.MaLT, x.SO_HD, chitiet = "Chi tiết", x.keys });
            dataGridView1.DataSource = dataSourceDGV1.OrderByDescending(x => x.ID_HD).ToList();
        ...
        private void frQLHoaDonThayThe_Load(object sender, EventArgs e)
        {
